...
# ---------------------------------------------------------------------------
# Statistical helpers for sampling
# ---------------------------------------------------------------------------
...
class Diffcheck:
    ...
    # ------------------------------------------------------------------
    # Sampled comparison (explicit opt-in; cross-database only)
    # ------------------------------------------------------------------

    def compare_sampled(
        self,
        sample_size: int = SAMPLED_DEFAULT_SIZE,
        confidence: float = SAMPLED_DEFAULT_CONFIDENCE,
    ) -> DiffRun:
    ...
    def compare(self) -> DiffRun:
        ...
        if self._left_config.get('use_sampling'):  # type: ignore[attr-defined]
            ...
            return self.compare_sampled(sample_size=sample_size, confidence=confidence)

-- DiMer test fixture: CockroachDB side (source B / "target")
-- CockroachDB is PostgreSQL wire-compatible; apply with:
-- cockroach sql --host=$COCKROACH_HOST --insecure -d $COCKROACH_DATABASE -f cockroachdb_customers.sql
-- (drop --insecure and add --certs-dir=... for a secured cluster)
...
-- Row 9: only in CockroachDB -> ADDED (missing from Postgres)
INSERT INTO customers (id, name, email, amount, status, created_at) VALUES
...
-- Row 12: only in CockroachDB -> ADDED
INSERT INTO customers (id, name, email, amount, status, created_at) VALUES

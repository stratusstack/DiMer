-- DiMer test fixture: Snowflake side (source B / "target")
-- Apply via SnowSQL: snowsql -a $SNOWFLAKE_ACCOUNT -u $SNOWFLAKE_USER
--   -d $SNOWFLAKE_DATABASE -s $SNOWFLAKE_SCHEMA -w $SNOWFLAKE_WAREHOUSE -f snowflake_customers.sql
-- Pair with testdata/postgres_customers.sql (source A) to exercise ADDED/DELETED/MODIFIED.
-- Note: Snowflake upper-cases unquoted identifiers (see IDENTIFIER_CASE="upper"
-- in the connector); this DDL relies on that default, so leave identifiers unquoted.
...
-- Row 9: only in Snowflake -> ADDED (missing from Postgres)
INSERT INTO customers (id, name, email, amount, status, created_at) VALUES
...
-- Row 12: only in Snowflake -> ADDED
INSERT INTO customers (id, name, email, amount, status, created_at) VALUES

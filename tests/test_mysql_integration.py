"""Integration tests for MySQL connector with real database connection."""
...
def real_mysql_config():
    """Real MySQL configuration from environment variables."""
    ...
    if not all([config.username, config.password]):
        pytest.skip("MySQL credentials not provided in environment variables")

    return config


# Target table used for table-level tests; can be overridden via env
TARGET_TABLE = os.getenv("MYSQL_TABLE", "")


@pytest.mark.requires_db
class TestMySQLIntegration:
    """Integration tests for real MySQL connections."""

    def test_connection_methods_priority(self, real_mysql_config):
        """Test that connection methods are tried in the right order."""
        connector = MySQLConnector(real_mysql_config)
        methods = connector.get_connection_methods()

        assert methods[0] == ConnectionMethod.MYSQL_CONNECTOR
        assert methods[1] == ConnectionMethod.PYMYSQL
        assert ConnectionMethod.SQLALCHEMY in methods

    def test_real_connection_establishment(self, real_mysql_config):
        """Test establishing a real connection to MySQL."""
        connector = MySQLConnector(real_mysql_config)
    ...
    def test_factory_creates_mysql_connector(self, real_mysql_config):
        """Test that the factory creates a MySQL connector correctly."""
        from dimer.core.factory import _auto_register_connectors

        _auto_register_connectors()

        connector = ConnectorFactory.create_connector("mysql", real_mysql_config)

        assert isinstance(connector, MySQLConnector)
        assert connector.connection_config == real_mysql_config

    def test_list_schemas(self, real_mysql_config):
        """Test listing schemas (databases) in the MySQL server."""
        connector = MySQLConnector(real_mysql_config)
        ...
        try:
            ...
            print(f"Found schemas (databases): {schemas}")

        finally:
            connector.close()

    def test_list_tables_in_database(self, real_mysql_config):
        """Test listing tables in the configured database."""
        connector = MySQLConnector(real_mysql_config)

        try:
            connector.connect()
            tables = connector.list_tables(schema_name=real_mysql_config.database)

            assert isinstance(tables, list)
            print(f"Tables in '{real_mysql_config.database}': {tables}")
    ...
    def test_target_table_metadata(self, real_mysql_config):
        """Test retrieving metadata for the target table."""
        if not TARGET_TABLE:
            pytest.skip("MYSQL_TABLE env var not set; skipping table metadata test")
        ...
        try:
            ...
            database = real_mysql_config.database
            tables = connector.list_tables(schema_name=database)

            if TARGET_TABLE not in tables:
                pytest.skip(
                    f"Table '{TARGET_TABLE}' not found in database '{database}'"
                )

            metadata = connector.get_table_metadata(
                TARGET_TABLE, schema_name=database
            )
    ...
    def test_sample_data_from_target_table(self, real_mysql_config):
        """Test retrieving sample data from the target table."""
        if not TARGET_TABLE:
            pytest.skip("MYSQL_TABLE env var not set; skipping sample data test")
        ...
        try:
            ...
            sample_df = connector.get_sample_data(
                TARGET_TABLE, limit=5, schema_name=database
            )
    ...
    def test_query_execution(self, real_mysql_config):
        """Test executing a basic query on the MySQL server."""
        connector = MySQLConnector(real_mysql_config)
        ...
        try:
            ...
            result = connector.execute_query(
                "SELECT VERSION() AS mysql_version, USER() AS current_user, "
            ...
            # Column names may be lowercase or uppercase depending on the driver
            columns_lower = [c.lower() for c in df.columns]
            assert "mysql_version" in columns_lower
    ...
    def test_query_execution_on_target_table(self, real_mysql_config):
        """Test executing a COUNT query on the target table."""
        if not TARGET_TABLE:
            pytest.skip("MYSQL_TABLE env var not set; skipping table query test")
        ...
        try:
            ...
            result = connector.execute_query(
                f"SELECT COUNT(*) AS total_rows FROM `{database}`.`{TARGET_TABLE}`"
            )
            ...
            # Column name may be lowercase or uppercase
            col = df.columns[0]
            total_rows = df.iloc[0][col]
    ...
    def test_connection_metrics(self, real_mysql_config):
    ...
    def test_context_manager_usage(self, real_mysql_config):
        """Test using the connector as a context manager."""
        with MySQLConnector(real_mysql_config) as connector:
            ...
            assert not df.empty
            # Column name may be uppercase or lowercase depending on the driver
            col = df.columns[0]
            assert df.iloc[0][col] == 1

        # Connection should be closed after exiting context
        assert connector.connection is None

    @pytest.mark.slow
    def test_different_connection_methods(self, real_mysql_config):
        """Test each connection method individually."""
        methods_to_test = [
            (ConnectionMethod.MYSQL_CONNECTOR, "_connect_mysql_connector"),
            (ConnectionMethod.PYMYSQL, "_connect_pymysql"),
        ...
        for method, connect_fn in methods_to_test:
            ...
            try:
                test_connector = MySQLConnector(real_mysql_config)
